package com.earthgame.idle.domain.milestones
 ...
 *
 * Where `Events.kt` holds *random* events — short-lived multiplier swings
 * rolled by chance — this file holds *deterministic* ones: headlines that fire
 * the first time a run crosses a specific threshold. They carry no mechanical
 * effect at all. Their whole job is to narrate what the player's civilization is
 * doing to the world, so a week-long run reads as a story of consequences
 * rather than a rising number.
 *
 * The set deliberately spans the entire run. Early headlines are triggered by
 * *technology*, because a handful of campfires genuinely cannot move a planet's
 * atmosphere and pretending otherwise would be a lie the rest of the simulation
 * does not tell; the consequences reported there are local and regional.
 * Climate headlines take over in the back half, once emissions are large enough
 * to show up in the global numbers, and they escalate from treaty thresholds to
 * obituaries.
 *
 * Each milestone fires at most once per run, tracked in
 * `GameState.milestonesTriggered`, which resets with every new Earth.
 */
enum class MilestoneCategory { INDUSTRY, CLIMATE, ECOLOGY, SOCIETY, APOCALYPSE }
...
private fun co2Ppm(state: GameState): Double = gasDisplayConcentration(GasId.CO2, state.atmosphere)

private fun owns(state: GameState, id: String): Boolean = (state.techOwned[id] ?: 0) > 0

/** Ordered by roughly when they fire, which is also the order the feed seeds them in. */
val MILESTONES: List<MilestoneDefinition> = listOf(
    MilestoneDefinition(
        id = "first_smoke",
        ...
        icon = "🔥",
        category = MilestoneCategory.INDUSTRY,
        condition = { state -> owns(state, "controlled_fire") },
    ),
    MilestoneDefinition(
        id = "the_first_field",
        ...
        icon = "🌾",
        category = MilestoneCategory.INDUSTRY,
        condition = { state -> owns(state, "farming") },
    ),
    MilestoneDefinition(
        id = "industrial_era",
        ...
        icon = "🏭",
        category = MilestoneCategory.INDUSTRY,
        condition = { state -> owns(state, "steam_engine") },
    ),
    MilestoneDefinition(
        id = "coal_smog",
        ...
        icon = "🌫️",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> owns(state, "coal_mining") },
    ),
    MilestoneDefinition(
        id = "river_ran_black",
        ...
        icon = "🏭",
        category = MilestoneCategory.ECOLOGY,
        condition = { state -> owns(state, "steel_production") },
    ),
    MilestoneDefinition(
        id = "assembly_line",
        ...
        icon = "⚙️",
        category = MilestoneCategory.INDUSTRY,
        condition = { state -> owns(state, "factories") },
    ),
    MilestoneDefinition(
        id = "grid_switched_on",
        ...
        icon = "💡",
        category = MilestoneCategory.INDUSTRY,
        condition = { state -> owns(state, "coal_power_plant") },
    ),
    MilestoneDefinition(
        id = "oil_age",
        ...
        icon = "🛢️",
        category = MilestoneCategory.INDUSTRY,
        condition = { state -> owns(state, "oil_drilling") },
    ),
    MilestoneDefinition(
        id = "the_automobile",
        ...
        icon = "🚗",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> owns(state, "automobile") },
    ),
    MilestoneDefinition(
        id = "ozone_hole",
        ...
        icon = "🧴",
        category = MilestoneCategory.ECOLOGY,
        condition = { state -> owns(state, "cfcs") },
    ),
    MilestoneDefinition(
        id = "megacity",
        ...
        icon = "🏙️",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> owns(state, "megacities") },
    ),
    MilestoneDefinition(
        id = "fracking_boom",
        ...
        icon = "⛽",
        category = MilestoneCategory.INDUSTRY,
        condition = { state -> owns(state, "fracking") },
    ),
    MilestoneDefinition(
        id = "factory_farms",
        ...
        icon = "🐄",
        category = MilestoneCategory.ECOLOGY,
        condition = { state -> owns(state, "factory_farming") },
    ),
    MilestoneDefinition(
        id = "container_trade",
        ...
        icon = "🚢",
        category = MilestoneCategory.INDUSTRY,
        condition = { state -> owns(state, "container_ships") },
    ),
    MilestoneDefinition(
        id = "the_internet",
        ...
        icon = "🌐",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> owns(state, "internet") },
    ),
    MilestoneDefinition(
        id = "data_centers",
        ...
        icon = "🖥️",
        category = MilestoneCategory.INDUSTRY,
        condition = { state -> owns(state, "data_centers") },
    ),
    MilestoneDefinition(
        id = "jet_age",
        ...
        icon = "✈️",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> owns(state, "commercial_aviation") },
    ),
    MilestoneDefinition(
        id = "co2_300",
        ...
        icon = "📈",
        category = MilestoneCategory.CLIMATE,
        condition = { state -> co2Ppm(state) >= 300 },
    ),
    MilestoneDefinition(
        id = "co2_350",
        ...
        icon = "🌡️",
        category = MilestoneCategory.CLIMATE,
        condition = { state -> co2Ppm(state) >= 350 },
    ),
    MilestoneDefinition(
        id = "paris_1_5",
        ...
        icon = "🌍",
        category = MilestoneCategory.CLIMATE,
        condition = { state -> state.temperatureAnomalyC >= 1.5 },
    ),
    MilestoneDefinition(
        id = "co2_400",
        ...
        icon = "📊",
        category = MilestoneCategory.CLIMATE,
        condition = { state -> co2Ppm(state) >= 400 },
    ),
    MilestoneDefinition(
        id = "amazon_burning",
        ...
        icon = "🌳",
        category = MilestoneCategory.ECOLOGY,
        condition = { state -> state.temperatureAnomalyC >= 2 },
    ),
    MilestoneDefinition(
        id = "reef_bleached",
        ...
        icon = "🪸",
        category = MilestoneCategory.ECOLOGY,
        condition = { state -> state.oceanPh <= 8.05 },
    ),
    MilestoneDefinition(
        id = "arctic_ice_free",
        ...
        icon = "🧊",
        category = MilestoneCategory.CLIMATE,
        condition = { state -> state.temperatureAnomalyC >= 3 },
    ),
    MilestoneDefinition(
        id = "crop_failure",
        ...
        icon = "🌾",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> state.habitability.factors.agriculture <= 0.75 },
    ),
    MilestoneDefinition(
        id = "wet_bulb",
        ...
        icon = "🥵",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> state.temperatureAnomalyC >= 4 },
    ),
    MilestoneDefinition(
        id = "sea_level_cities",
        ...
        icon = "🌊",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> state.seaLevelRiseMeters >= 1 },
    ),
    MilestoneDefinition(
        id = "permafrost_thaw",
        ...
        icon = "💨",
        category = MilestoneCategory.CLIMATE,
        condition = { state -> state.temperatureAnomalyC >= 5 },
    ),
    MilestoneDefinition(
        id = "insect_collapse",
        ...
        icon = "🐝",
        category = MilestoneCategory.ECOLOGY,
        condition = { state -> state.habitability.factors.biodiversity <= 0.6 },
    ),
    MilestoneDefinition(
        id = "amazon_savanna",
        ...
        icon = "🪵",
        category = MilestoneCategory.ECOLOGY,
        condition = { state -> state.temperatureAnomalyC >= 6 },
    ),
    MilestoneDefinition(
        id = "co2_500",
        ...
        icon = "📉",
        category = MilestoneCategory.CLIMATE,
        condition = { state -> co2Ppm(state) >= 500 },
    ),
    MilestoneDefinition(
        id = "gulf_stream",
        ...
        icon = "🌀",
        category = MilestoneCategory.CLIMATE,
        condition = { state -> state.temperatureAnomalyC >= 8 },
    ),
    MilestoneDefinition(
        id = "climate_refugees",
        ...
        icon = "🚶",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> state.habitability.fraction <= 0.4 },
    ),
    MilestoneDefinition(
        id = "co2_1000",
        ...
        icon = "🔥",
        category = MilestoneCategory.CLIMATE,
        condition = { state -> co2Ppm(state) >= 1000 },
    ),
    MilestoneDefinition(
        id = "ocean_dead_zones",
        ...
        icon = "🐟",
        category = MilestoneCategory.ECOLOGY,
        condition = { state -> state.oceanPh <= 7.8 },
    ),
    MilestoneDefinition(
        id = "coastlines_gone",
        ...
        icon = "🗺️",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> state.seaLevelRiseMeters >= 5 },
    ),
    MilestoneDefinition(
        id = "grid_failure",
        ...
        icon = "⚡",
        category = MilestoneCategory.SOCIETY,
        condition = { state -> state.temperatureAnomalyC >= 15 },
    ),
    MilestoneDefinition(
        id = "last_broadcast",
        ...
        icon = "📻",
        category = MilestoneCategory.APOCALYPSE,
        condition = { state -> state.habitability.fraction <= 0.08 },
    ),
    MilestoneDefinition(
        id = "venus_watch",
        ...
        icon = "🪐",
        category = MilestoneCategory.APOCALYPSE,
        condition = { state -> state.temperatureAnomalyC >= 25 },
    ),
)

val MILESTONE_BY_ID: Map<String, MilestoneDefinition> = MILESTONES.associateBy { it.id }

/**
 * Every milestone whose condition is met and which has not already fired this
 * run, in definition order. Pure — the caller decides what to do with them
 * (append to the feed, raise a toast).
 */
fun checkMilestones(state: GameState): List<MilestoneDefinition> =
    MILESTONES.filter { state.milestonesTriggered[it.id] != true && it.condition(state) }
